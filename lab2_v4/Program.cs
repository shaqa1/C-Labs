...
namespace lab2_v4
{
    ...
    {
        private delegate void Output(string URI, int Depth, string Email);
        private event Output OutputEvent;
        private void SearchForContacts(string URI, int Depth)
        {
            //try
            //{
                ...
                {
                    OutputEvent?.Invoke(URI, Depth, emailmatch.Value);
                ...
                if (Depth > 0)
                {
                    ...
                    while (hrefmatch.Success)
                    {
                        ...
                        if (href.Contains("susu.ru/ru/") && href != URI) SearchForContacts(href, Depth - 1);
        ...
        class Program
        {
            ...
            static void Main()
            {
                ContactsSearcher webanalysis = new ContactsSearcher();
                ...
                wrongfileoutputextension:
                    ...
                    webanalysis.OutputEvent += Print;
                    webanalysis.OutputEvent += Display;
                }
                else webanalysis.OutputEvent += Display;
                ClearDraw("Press any key to start scanning...\n");
                Console.ReadKey();
                webanalysis.SearchForContacts(URI, ScanDepth);
